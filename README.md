...
This section is here because **honest assessment is more valuable than inflated metrics**. Real ML engineering involves understanding not just what works, but what doesn't.

### 1. Synthetic Priority Labels

**The issue:** The P0–P3 priority labels are generated by my own rule-based system (keyword detection + category-based defaults). The model is partially learning my heuristics, not real urgency.

**Impact:** The 91.5% priority F1 is misleading — the model is rediscovering my rules. In production, I would use human-labeled urgency from past escalation patterns, customer satisfaction scores, or SLA breach data.

**Why I did this:** No real labeled urgency data exists in Banking77. The rule-based system serves as a proof-of-concept for the multi-task architecture.

### 2. Softmax Overconfidence on Out-of-Distribution Inputs

**The issue:** The model produces high-confidence predictions even on nonsense input (e.g., "asdf qwerty xyz" → General Inquiry, 96% confidence). This is a known limitation of softmax-based classifiers.

**Mitigation in this project:** Confidence threshold of 0.85 with human review fallback (see error analysis).

**Real production fix:** Temperature scaling for calibration, explicit OOD (out-of-distribution) detector, or energy-based confidence scores.

### 3. Label Noise from Category Consolidation

**The issue:** Error analysis revealed that some "wrong" predictions are actually correct — they're errors in my label consolidation. For example, "I want a new card" was dumped into General Inquiry, but the model correctly predicts Card Services.

**Impact:** True model accuracy is likely higher than 87.5% (baseline) and 90.6% (DistilBERT) once label noise is accounted for.

**How I would fix:** Re-label the 7 unmapped Banking77 categories (e.g., `disposable_card_limits`, `getting_spare_card`) into their semantically correct consolidated categories instead of defaulting to General Inquiry.

### 4. Fair Benchmark Numbers (Updated)

The original README had a misleading benchmark that mixed PyTorch GPU and ONNX CPU times. The correct **same-hardware (CPU) benchmark**:

| Model | CPU Inference | Speedup |
|:------|:-:|:-:|
| TF-IDF + LogReg | 1.14 ms | baseline |
| BiLSTM (PyTorch) | 9.16 ms | — |
| DistilBERT (PyTorch) | 78.00 ms | 1.0x |
| **DistilBERT (ONNX)** | **38.01 ms** | **2.05x** |

ONNX provides **2x speedup over PyTorch on CPU** — the fair comparison for production deployment. PyTorch on GPU is faster than ONNX on CPU, but GPU instances cost 5–10x more in production.

### 5. No Real Production Validation

This project was built and validated on Banking77 + CLINC150 — public academic datasets. A real production system would need:

- **Real historical ticket data** with human-assigned categories
- **A/B testing** against current routing logic
- **Drift monitoring** with active model quality tracking (not just data drift)
- **Audit logging** of every prediction with human override tracking
- **Periodic retraining** triggered by drift signals
- **Authentication and rate limiting** on the API

### 6. Class-wise Performance is Uneven

| Performance | Categories |
|:------------|:-----------|
| Strong (F1 ≥ 0.90) | Account Access (0.93), Technical Issues (0.91) |
| Moderate (F1 0.85–0.90) | Card Services, Account Management, Refund & Dispute |
| Weak (F1 < 0.85) | Balance & Statement (0.83), Transfer & Transaction (0.86) |

The weakest categories overlap semantically — Balance & Statement and Transfer & Transaction both involve viewing money flow. Even human annotators would disagree on edge cases.

### 7. What Calibration Looks Like (The Good News)

The model is **well-calibrated**:
- Correct predictions: mean confidence **0.688**
- Wrong predictions: mean confidence **0.451**

This means confidence scores are meaningful — when the model is unsure, it's usually wrong. This validates the confidence threshold approach: with a 0.85 threshold, the auto-routed 22% of tickets achieve **96.9% accuracy**.
